<!--
  ...
  @Last Modified time: 2020/8/27
 -->
<template>
    <div class = "custom-tree-wrap" :ref="treeRefName" :class="treeClassName">
        <div
...
<script lang="ts">
    ...
    export default class CustomTree extends Vue {
        ...
        @Watch('$props.data', { deep: true, immediate: true})
        dataChange(newValue: any){
            if (newValue && newValue.length) {
                this.convertData=this.convert(newValue);
            }
        }
        ...
        public convertData = this.convert(this.$props.data);

        public rootStyle: any = {};

        initDomWidth(){
            let leafs: any=document.getElementsByClassName('leaf-node');
            leafs = Array.from(leafs);
            leafs=leafs.map((i: any)=>{
                return this.getOffset(i,'offsetLeft')
            })
            this.rootStyle={width:Math.max(...leafs)*1.5+'px'}
        }

        getOffset(obj: any, offsetDir: any){
